...
void Game::OnDraw(wxGraphicsContext *graphics, int width, int height)
{
    ...
    graphics->DrawRectangle(double(-Width) / 2 , 0, Width, Height);
    graphics->Clip(double(-Width) / 2 , 0, Width, Height);
...
{
    mMouseX = double(x - mXOffset) / mScale;
    mMouseY = double(y - mYOffset) / mScale;
}
