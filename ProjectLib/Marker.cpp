...
void Marker::Update(double elapsed)
{
    ...
    if (mLaunched)
    {
        auto hitSomething = game->HitTest(int(GetX()), int(GetY()));  // getting the hit place location
        ...
        if (xCoordinate > double(Game::Width) / 2 || xCoordinate < - double(Game::Width) / 2  )
        {
            mLaunched = false;
        }
        if (yCoordinate > double(Game::Height) || yCoordinate < 0)
        {
            mLaunched = false;
        }
        return;
...
void Marker::Draw(wxGraphicsContext *graphics)
{
    ...
    graphics->DrawBitmap(mItemBitmap, GetX() - double(tempImage.GetWidth()) / 2, GetY() - double(tempImage.GetHeight()) / 2,
